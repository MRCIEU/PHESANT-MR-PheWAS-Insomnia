	...
	scale_x_discrete(labels = function(outcome) str_wrap(outcome, width = 40))+
 	geom_text(aes(label = paste(format(round(b, digits=2), nsmall = 2),
  		" (",
        	...
        	")",
        	sep=""), y = -4.5, x = outcome, group = method), 
  	...
  	theme(axis.text.y=element_text(size=ts), legend.title = element_blank(), legend.position="bottom",
		axis.title.y=element_text(size=hs, vjust=10, face="bold"), axis.title.x=element_text(size=hs, vjust=-2, face="bold"),
...
ggsave("FP_MD.jpg", width = 15, height = 10)
...
forest <- ggplot(ORcats, aes(x=fct_reorder(outcome, outcome, .desc=TRUE), y=b, ymin=lower, ymax=upper, colour=method)) +
	geom_point(size=1, position = position_dodge(width=pdd)) +
	geom_errorbar(width=et, position = position_dodge(width=pdd)) +
	...
	geom_text(aes(label = paste(format(round(b, digits=2), nsmall = 2), 
		...
		format(round(upper, digits=2), nsmall = 2), 
		")", 
		sep=""), y = -1, x = outcome, group = method), 
		position = position_dodge(width = pdl), colour="black", size=ls) +
	  facet_grid(rows = vars(category), 
             scales = "free_y",
             space = "free_y",
             switch = "y") +
        coord_flip(ylim=c(-1.5, uplim), expand=0) +
	theme(axis.text.y=element_text(size=ts), legend.title = element_blank(), legend.position="bottom",
		axis.title.y=element_text(size=hs, vjust=10, face="bold"), axis.title.x=element_text(size=hs, vjust=-2, face="bold"),
        	strip.placement = "outside", strip.background = element_rect(fill = "white"), 
        	strip.text.y.left = element_text(size=ts, angle = 0, hjust=0, face="bold"),
		...
		plot.margin=unit(c(2,2,2,2), "cm"))+
		guides(colour = guide_legend(reverse = TRUE))
ggsave("FP_OR.jpg", width = 15, height = 25)
